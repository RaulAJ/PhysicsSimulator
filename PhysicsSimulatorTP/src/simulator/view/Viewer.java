...
class Viewer extends SimulationViewer {
			...
			public void keyPressed(KeyEvent e) {
				switch (e.getKeyChar()) {
					...
					case 'v':
						...
						break;
					case 'g':
						_selectedGroupIdx = (_selectedGroupIdx + 1) % (_groups.size() + 1);
	...
	private void showHelp(Graphics2D g) {
		...
		g.drawString("Scaling ratio: " + _scale, 10, 65);
		g.drawString("Selected Group: " + (_selectedGroup == null ? "All" : _selectedGroup), 10, 80);
	}
	...
	private void drawBodies(Graphics2D g) {
		...
		for (Body b : _bodies){
			if (isVisible(b)){
				...
				double x = _centerX +  b.getPosition().getX() / _scale;
				double y = _centerY -  b.getPosition().getY() / _scale;
				
				g.setColor(_gColor.get(b.getgId()));
				g.fillOval((int)x - 3,(int) y - 3, 10, 10);
				g.setColor(Color.BLACK);
				g.drawString(b.getId(), (int)x - 5,(int) y - 5);
				
				if (_showVectors){
					//Vectores velocidad
					double vx, vy;
					
					vx = x + b.getVelocity().direction().getX() * 15;
									
					vy = y + b.getVelocity().direction().getY() * 15;
					
					drawLineWithArrow(g, (int)x, (int)y, (int)vx,(int) vy, 5, 5, Color.BLUE, Color.BLUE);

					//vector fuerza
					double fx = 13 * b.getForce().direction().getX() + x;
					double fy = 13 * b.getForce().direction().getY() + y;
					drawLineWithArrow(g, (int)x, (int)y ,(int)fx, (int)fy, 5, 5,Color.RED, Color.RED);
				}
			}
		}
	...
	public void addGroup(BodiesGroup g) {
		...
		{
			_bodies.add(body);
			if(!_gColor.containsKey(body.getgId())) {
				_gColor.put(body.getgId(), _colorGen.nextColor());
			}
